...
def _assemble_template() -> str:
    ...
    assert "__DATA__" not in css, "CSS must not contain __DATA__ placeholder"
    assert "__POLL_MS__" not in css, "CSS must not contain __POLL_MS__ placeholder"
    assert "__LIVE_MODE__" not in css, "CSS must not contain __LIVE_MODE__ placeholder"
...
def build_html(data: dict | None = None, *, poll_interval_ms: int = 0, live: bool = False) -> str:
    ...
    interval = str(max(1000, int(poll_interval_ms or 0)))
